<template>
    ...
    >
      ...
      <div
      v-if="previewImage"
      class="w-full h-full bg-cover bg-center cursor-pointer"
    ...
    <div v-else class="text-center cursor-pointer" @click="triggerFileInput">
      ...
      <!-- Edit Icon -->
      <button
        class="absolute bottom-0 right-0 w-8 h-8 bg-gray-800 text-white rounded-full flex items-center justify-center hover:bg-gray-700"
        @click="triggerFileInput"
      >
        ✎
      </button>
      ...
      <div v-if="newImage" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50">
        <div class="bg-white rounded-lg shadow-lg p-4 max-w-lg w-full">
          ...
          <div class="flex justify-between mt-4">
            ...
            <button class="px-4 py-2 bg-dark text-white rounded" @click="croppingConfirmed">Confirm</button>
          </div>
        </div>
      </div>
    </div>
  ...
  const props = defineProps({
    modelValue: { type: String, default: "" }, // ✅ Use v-model
  ...
  const croppingConfirmed = async () => {
    ...
    try {
      ...
      emit("update:modelValue", previewImage.value); // ✅ Sync with parent component
    } catch (error) {
      console.error(error);
    }
    cancelPick(); 
