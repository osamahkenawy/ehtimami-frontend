  ...
  <div class="h-fit">
    ...
    <div class="flex gap-5 relative h-fit sm:min-h-0"
      ...
      <div class="panel p-4 flex-none max-w-xs w-full absolute xl:relative z-10 space-y-4 hidden xl:block overflow-hidden h-fit"
        ...
        <Chip
          :content="selectedStudent?.user?.is_verified ? t('user.verified') : t('user.unverified')"
          :textColor="selectedStudent?.user?.is_verified ? '#00ab55' : '#e7515a'"
          :borderColor="selectedStudent?.user?.is_verified ? '#00ab55' : '#e7515a'"
          :headerBackgroundColor="selectedStudent?.user?.is_verified ? '#e6f9f0' : '#ffecec'"
          class="ltr:ml-auto rtl:mr-auto"
        />
        <UserProfileCard
          v-if="selectedStudent"
          :image="selectedStudent?.user?.profile?.avatar"
          :name="selectedStudent?.user?.firstName + ' ' + selectedStudent?.lastName"
          :gender="selectedStudent?.user?.profile?.gender"
          :first-name="selectedStudent?.user?.firstName"
          :last-name="selectedStudent?.user?.lastName"
          :email="selectedStudent?.user?.email"
          :phone="selectedStudent?.user?.phone"
          :dob="selectedStudent?.user?.profile?.birth_date"
          :address="selectedStudent?.user?.profile?.address"
        />
      ...
      <div class="panel p-0 flex-1 h-fit"> 
        <div class="p-6">
          <ProfileDetails v-if="activeSection === 'overview'" :user="selectedStudent"  @updated="loadUserProfile" />
          <AcademicSection v-else-if="activeSection === 'academic'" :student="selectedStudent" />
          <!-- <AcademicSection v-else-if="activeSection === 'academic'" :student="selectedStudent" />
...
import AcademicSection from "@/views/ehtimami/students/components/Profile/AcademicSection.vue";
