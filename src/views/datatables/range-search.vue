<template>
    <div>
        ...
        <div class="panel pb-1.5 mt-6">
            <div class="mb-4.5 flex md:items-center md:flex-row flex-col gap-5">
                <div class="flex items-center gap-5">
                    <div class="md:flex-auto flex-1">
                        <input v-model="minAge" type="text" class="form-input" placeholder="Minimum age..." @keyup.prevent="rangeChange()" />
                    </div>
                    <div class="md:flex-auto flex-1">
                        <input v-model="maxAge" type="text" class="form-input" placeholder="Maximum age..." @keyup.prevent="rangeChange()" />
                    </div>
                </div>
...
<script setup lang="ts">
    ...
    const rangeChange = () => {
        let dt = allRows.value;
        if (minAge.value != '' && minAge.value != null) {
            dt = dt.filter((d) => d.age >= Number(minAge.value));
        }
        if (maxAge.value != '' && maxAge.value != null) {
            dt = dt.filter((d) => d.age <= Number(maxAge.value));
        }
        rows.value = dt;
    };
